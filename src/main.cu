...
__global__ void smoothSignal(const float* input, float* output, int length) {
...
void processFile(const char* filename, const char* outname) {
    ...
    smoothSignal<<<blocks, BLOCK_SIZE>>>(d_input, d_output, length);
...
int main() {
    processFile("data/sample_signal.csv", "output/smoothed_signal.csv");
